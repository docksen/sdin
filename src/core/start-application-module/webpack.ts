...
export async function getWebpackEntry(module: SdinApplicationModule): Promise<EntryObject> {
  ...
  const hmrClient = withModulePath('webpack-hot-middleware/client?reload=true')
  for (let page of module.pages) {
    ...
    entry[page.name] = [hmrClient, index]
  }
...
/**
 * 尽可能使用本地的包（减少 CDN 包，去除网络带来的阻碍）
 */
export function getWebpackExternals(module: SdinApplicationModule) {
  const externals: Record<string, string> = {}
  for (let key in module.externals) {
    const pkgName = getDependenceName(key)
    if (!getDependenceVersion(module.pkg, pkgName)) {
      externals[pkgName] = module.externals[key]
    }
  }
  return externals
}
    ...
    use: [
      {
        ...
        options: {
          ...
          plugins: [
            ...
            withModulePath('react-refresh/babel')
          ]
        }
      }
    ]
